...
if [ $USE_CHARITY = "true" ]; then
	DOCKFILES=/local/input/dock6files
	tar -C /local/input -xf /local/input/dock6files.tgz
	DOCKEXEC=/bin/dock64
...
function cleanup {
	...
	complet=$(tail $JOB_DIR/dock.out | grep "Total elapsed time" | wc -l)
	sigusr1=$(tail $JOB_DIR/dock.out | grep "Program will terminate" | wc -l)
	nullres=$(tail $JOB_DIR/dock.out | grep "Molecules Processed" | awk '{print $1}')
	...
	nout=$RESUBMIT_COUNT
	if ! [ -f $OUTPUT/dock.out ] && [ "$success" = true ]; then
	...
	cp $JOB_DIR/dock.out $OUTPUT/dock.out$OUTPUT_SUFFIX.$nout
	cp $JOB_DIR/output_scored.mol2 $OUTPUT/output_scored$OUTPUT_SUFFIX.$nout.mol2
...
pushd $JOB_DIR/ > /dev/null 2>&1


awk_cmd='{if (substr($0, 1, 1) == "S" && NF==8 && length==47) print substr($0, 1, 35); else print $0}'

log "starting DOCK vers=6"

	if [ $USE_DB2_TGZ = "true" ]; then
		# if [ $vers = "3.7" ]; then
		# 	python3 $TARSTREAM_SCRIPT $INPUT_FILES | awk "$awk_cmd"
		# else
			python3 $TARSTREAM_SCRIPT $INPUT_FILES > $JOB_DIR/ligands.db2
		# fi
	elif [ $USE_DB2 = "true" ]; then
		# if [ $vers = "3.7" ]; then
		# 	zcat -f $INPUT_FILES | awk "$awk_cmd"
		# else
			zcat -f $INPUT_FILES > $JOB_DIR/ligands.db2
		# fi
	fi
echo "ligands.db2" > split_database_index
log "ligands.db2 created, size=$(stat -c%s $JOB_DIR/ligands.db2), path=$JOB_DIR/ligands.db2"
