...
RUNDOCK_PATH=$BINDIR/rundock6.bash
...
for input in $($get_input_cmd); do
	if [ $RESUBMIT_COUNT -eq 0 ]; then # first run don't bother checking file existence
		echo $input
		njobs=$((njobs+1))
	elif ! [ -f $EXPORT_DEST/$input/dock.out.0 ]; then
		echo $input
		njobs=$((njobs+1))
	elif ! [ -f $EXPORT_DEST/$input/output_scored.0.mol2 ]; then
		#rm $EXPORT_DEST/$input/*
		echo $input
		njobs=$((njobs+1))
	elif [ -f $EXPORT_DEST/$input/dock.out.0 ] && [ -f $EXPORT_DEST/$input/restart ]; then
		echo $input
		njobs=$((njobs+1))
		nrestart=$((nrestart+1))
	fi
